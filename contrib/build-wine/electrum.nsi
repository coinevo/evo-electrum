...
;Variables
  ...
  !define PRODUCT_NAME "Evo-Electrum"
  !define PRODUCT_WEB_SITE "https://github.com/coinevo/evo-electrum"
  !define PRODUCT_PUBLISHER "Evo Team"
...
Section
  ...
  CreateShortCut "$DESKTOP\${PRODUCT_NAME}.lnk" "$INSTDIR\Evo-electrum-${PRODUCT_VERSION}.exe" ""

  ;Create start-menu items
  DetailPrint "Creating start-menu items..."
  CreateDirectory "$SMPROGRAMS\${PRODUCT_NAME}"
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\Uninstall.lnk" "$INSTDIR\Uninstall.exe" "" "$INSTDIR\Uninstall.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME}.lnk" "$INSTDIR\Evo-electrum-${PRODUCT_VERSION}.exe" "" "$INSTDIR\Evo-electrum-${PRODUCT_VERSION}.exe" 0
  CreateShortCut "$SMPROGRAMS\${PRODUCT_NAME}\${PRODUCT_NAME} Testnet.lnk" "$INSTDIR\Evo-electrum-${PRODUCT_VERSION}.exe" "--testnet" "$INSTDIR\Evo-electrum-${PRODUCT_VERSION}.exe" 0


  ;Links evo: URI's to Electrum
  WriteRegStr HKCU "Software\Classes\evo" "" "URL:evo Protocol"
  WriteRegStr HKCU "Software\Classes\evo" "URL Protocol" ""
  WriteRegStr HKCU "Software\Classes\evo" "DefaultIcon" "$\"$INSTDIR\electrum.ico, 0$\""
  WriteRegStr HKCU "Software\Classes\evo\shell\open\command" "" "$\"$INSTDIR\Evo-electrum-${PRODUCT_VERSION}.exe$\" $\"%1$\""
...
Section "Uninstall"
  ...
  DeleteRegKey HKCU "Software\Classes\evo"
