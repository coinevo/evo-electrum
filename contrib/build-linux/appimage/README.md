...
2. Build image
    ...
    ```
    $ cd evo-electrum
    $ sudo docker build -t evo-electrum-appimage-builder-img contrib/build-linux/appimage
    ```
...
3. Build binary
    ...
    ```
    $ sudo docker run -it \
        --name evo-electrum-appimage-builder-cont \
        -v $PWD:/opt/electrum \
        --rm \
        --workdir /opt/electrum/contrib/build-linux/appimage \
        evo-electrum-appimage-builder-img \
        ./build.sh
    ```
...
Execute the binary as follows: `./Evo-electrum*.AppImage --appimage-extract`
